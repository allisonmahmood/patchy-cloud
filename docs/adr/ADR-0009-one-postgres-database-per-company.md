...
`withCompany` supplies a typed company-connection capability. `withPatchLock` requires that capability and supplies a patch-lock capability tied to its patch id and transaction. Inventory mutations require the latter and reject a mismatched patch id before writing; they do not quietly start independent transactions. Raw SQL remains available inside the company lease for resource DDL and must follow the same outer lock protocol.
