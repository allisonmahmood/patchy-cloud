...
Internal skills sit outside `skills/` (which the CLI build copies wholesale) and carry `metadata.internal`: the operator-only `.agents/skills/patchy-mint-token`, and the `/code-review` review specs `.agents/skills/effect-service-conventions` and `.agents/skills/ui-consistency`.
