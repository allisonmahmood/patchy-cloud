...
Internal skills sit outside `skills/` (which the CLI build copies wholesale) and carry `metadata.internal`: the Patchy Cloud operator's `.agents/skills/patchy-mint-token`, the local-instance loop `.agents/skills/patchy-dev-loop`, and the `/code-review` review specs `.agents/skills/effect-service-conventions` and `.agents/skills/ui-consistency`.
