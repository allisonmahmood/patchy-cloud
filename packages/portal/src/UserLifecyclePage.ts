...
const notice = (message?: string) =>
  message === undefined
    ? ""
    : `<div class="note note-refused" role="alert">${escapeHtml(message)}</div>`;
  ...
  const damage =
    action === "deactivate"
      ...
      : warnings
          .map(
            (warning) =>
              `<li>${patchName(warning.name)} will serve, but it will error when it reads these tables until their sources are restored too:<ul>${warning.sources.map((source) => `<li>${patchName(source.name ?? source.patchId)} / ${patchName(source.table)}: ${escapeHtml(source.state)}</li>`).join("")}</ul></li>`
          )
          .join("");
  const summary =
    selected.length === 0
...
export const handle = Effect.fn("UserLifecyclePage.handle")(function* (id: string, action: Action) {
  ...
  const refuse = (message: string, status: number) =>
    respond(`${notice(message)}<p><a href="/company">Back to Company</a></p>`, status);
  if (viewer.role !== "admin")
    return refuse("Only an admin can manage this company. Nothing was done.", 403);
  const run = Effect.gen(function* () {
    // Match Patches' dependency lock, then lock rows before Users takes its user lock.
    // Reassign takes a patch lock before its target-user lock as well.
    yield* sql`SELECT pg_advisory_xact_lock(hashtextextended('patchy:dependencies:' || ${viewer.company.id}, 0))`;
    yield* sql`SELECT id FROM patches WHERE company_id = ${viewer.company.id} ORDER BY id FOR UPDATE`;
  ...
  return yield* sql.withTransaction(run).pipe(
    Effect.catchTags({
      ...
      HttpServerError: () =>
        Effect.succeed(refuse("Check the submitted form and try again. Nothing was done.", 422))
    })
  );
});
