...
**Analytics event**:
A server-side business moment: a patch created, updated, deleted or purged, or a machine token minted. It carries ids, sizes, counts and states, never a visit, reader address, page content, filename or URL; reporting is optional and its failure never fails the caller's request.
...
Who an event belongs to: the user who acted, or the instance itself for a deletion sweep. The reporting layer creates no person profile and reports no reader identity.
