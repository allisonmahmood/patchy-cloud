...
Call handlers with `patchy.server.<module>.<export>(args)` from an event handler or a `useEffect`; hold results in `useState` or a signal; read again after a mutation (there is no `.subscribe` or `useQuery` in this release). Branch on `isHandlerError(error, "<code>")` for a handler's own errors and treat `PatchyError`s as the platform's. Native form submission is blocked in the frame: use `type="button"` and an Enter handler that calls the handler, with `event.preventDefault()` on the form.
