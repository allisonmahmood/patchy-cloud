...
1. Run `pnpm patchy list connections` and choose a connected handle by description. `list connections --all` adds offered integrations and their state, not permission to use a disconnected source. Inspect `pnpm patchy list connections <handle> --json` for its immutable schema snapshot and `takenAt` before choosing relations, keys and types. A null snapshot means unavailable, not an empty database. An admin connects, reconnects or refreshes schema at `/company/connections`; ask them to use the browser form rather than supply you a connection string.
