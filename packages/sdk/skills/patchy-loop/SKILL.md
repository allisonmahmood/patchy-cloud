...
4. Test with `pnpm patchy dev` when the release provides the local runtime: exercise an insert and a list through the actual local shell, then inspect the result. This release lays down the repo and client; the dev runtime, browser broker and repo publishing are separate work. If the command is unavailable, report that boundary. A standalone Vite preview is not proof of a working Patchy runtime; do not replace it with production calls or a homemade transport.
