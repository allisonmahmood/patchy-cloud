...
2. Read `patchy.config.ts`. It defines owned tables and file stores and declares connections and shared tables in `uses`. On tier 2, `server/*.ts` holds the handlers the browser calls as `patchy.server.<file>.<export>`; read `../patchy-server/SKILL.md` before writing one. A new file under `server/` needs `pnpm patchy refresh`; editing an existing one does not. Read `../patchy-tables/SKILL.md` for rows, `../patchy-files/SKILL.md` for bytes, and the declaration's skill and generated context before using it. Import the generated client from `patchy/_generated/client.ts`; in `src/main.ts`, use `import { patchy } from "../patchy/_generated/client.js"`.
