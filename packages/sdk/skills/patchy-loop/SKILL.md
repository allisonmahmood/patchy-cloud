...
4. Test with `pnpm patchy dev` when the release provides the local runtime: exercise an insert and a list through the actual local shell, then inspect the result. The hosted browser broker and repo publishing are available; the local dev runtime remains separate work. If `dev` is unavailable, report that boundary. A standalone Vite preview is not proof of a working Patchy runtime; do not replace it with production calls or a homemade transport.
