/// <reference lib="dom" />
// PROTOTYPE for #314 round 3: see the subscription stream below (lifted from #313).
...
// PROTOTYPE for #314 round 3: the #313 subscription stream lifted in, admitting tier 2 queries.
import {
  RuntimeRequest,
  RuntimeFailure,
  runtimeOperations,
    ...
    try {
      ...
      try {
        if (op === "route.set") path = routePath(decodeRoute(message.args).path);
          ...
          // PROTOTYPE for #314 round 3: a tier 2 query rides `server.call` (a mutation for the
          // Origin check); the server refuses a handler that is not a query.
          if (runtimeOperations[read.op].kind !== "read" && read.op !== "server.call")
            throw invalid();
