...
contain a query, fragment, traversal or another origin. Routes are reported
percent-decoded, whether they arrive from the initial link, an acknowledged set
or back/forward: `set("/caf%C3%A9")` and `set("/café")` both read back as
`/café`. Routing works on both company and public patches. `client.route.subscribe(listener)` returns an
