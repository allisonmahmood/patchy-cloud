...
- **Company resources.** Read [ADR-0009](docs/adr/ADR-0009-one-postgres-database-per-company.md) before adding a company-database caller: take the platform patch-row lock first; open a company transaction under `withPatchLock` only for existing inventory or an operation introducing resources. Keep primitive-free patches off this path.
