...
- **Migrations.** Spread every migration into the server's migrator run, the dev runner's, and the vitest template's. There are eight records: `0001` companies, `0002` auth, `0003` patches, `0004` companies' invites-expiry, `0005` company-database, `0006` runtime, `0007` integrations, `0008` patches-lifecycle. Allocate ids in landing order: the migrator only applies ids above its ledger's highest id and never backfills gaps.
...
TypeScript 7 compiles and checks the workspace; `@effect/tsgo` adds the Effect diagnostics configured in `tsconfig.base.json`. `effect-tsgo patch` runs from `prepare` and `typecheck`. Oxlint runs the typed lint policy and the custom rules under `eslint/`; ESLint supplies the rule unit tests and the import-boundary rule. Read `docs/DEVELOPMENT.md`'s **TypeScript tooling** before changing the compiler, checker or linter. A Node API with no Effect equivalent is allowed per file with `// @effect-diagnostics <rule>:off` and a reason. Synchronous crypto helpers explain their Node imports in local diagnostic comments. `pnpm lint` runs the repo's own rules from `eslint/`: namespace imports for Effect and service modules, no manual runtimes in tests, no Schema compiles in function bodies. `/code-review` loads the service review spec above whenever a diff touches an Effect service.
