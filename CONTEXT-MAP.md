...
- [Hosting](./apps/server/CONTEXT.md) — receives uploads and serves published pages; owns `@patchy/db`, `@patchy/config`
...
- **Auth → Analytics, Limits, SQL**: minting reports through `Analytics`, spends the per-minute mint limit through `Limits`, and reads and writes its tables through the `SQL` client; `JsonTokens` in `@patchy/db` stands in for the Postgres tokens layer until the `patches` port deletes it
- **Hosting → Analytics, Limits, Content store**: the server's emitters report through `Analytics`, its per-minute limits spend attempts through `Limits`, and its upload contract and expiry sweep put, get and delete a patch's bytes through `ContentStore`; all three are consumed through one runtime seam until the server itself moves onto Effect
