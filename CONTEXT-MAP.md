...
- [Patches](./packages/patches/CONTEXT.md): `packages/patches`. Patches and immutable versions, company-scoped names, manifests and replay-safe publish keys, tier 0 and tier 1 bundle admission, additive table and store provisioning, company-readable inventory and the shared discovery query, ownership, sharing, retire, delete, restore, rollback, reassignment, descriptions, visits, the owner quota, the deletion sweep and the `patches` API group. Agent discovery, owner lifecycle CLI commands and inline portal management are built.
...
- [Portal](./packages/portal/CONTEXT.md): `packages/portal`. The signed-in landing at `/`, the patch card, versions, inline description, sharing and rollback, confirmation pages for retire, delete, restore with off sources and reassign, and the company user's deactivation/reactivation pick and confirmation pages, over Patches, Companies and Auth.
...
- [Publishing](./packages/patchy/CONTEXT.md) — `packages/patchy` and `packages/sdk`, sharing one glossary:
  - `packages/patchy`: the single `patchy` package's CLI, config builders, browser client and local PGlite dev runtime; patch-repo initialization and publishing, transactional refresh, company tool and data-source discovery through `list`, declaration editing, managed files, global and project skills, and fixtures
...
- **Portal → Patches, Companies, Auth, `core`**: reads the company inventory, manages patches and coordinates user deactivation/reactivation with selected patch changes in one transaction, admits company viewers through Auth, and composes the shared app shell.
