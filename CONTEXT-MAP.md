...
- [Integrations](./packages/integrations/CONTEXT.md) — `packages/integrations`. Company Postgres connections, encrypted credentials, immutable schema snapshots and admin connection pages. Publish binds connected identities and exact metadata revisions; operation handlers and generated clients follow separately
...
- **Integrations → Runtime, Auth**: records discovery with the acting admin and mounts connection pages behind the existing session and company-role boundary; no integration runtime handlers are admitted yet
