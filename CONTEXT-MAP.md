...
- [Portal](./packages/portal/CONTEXT.md): `packages/portal`. The signed-in landing at `/`, the patch card, versions and inline description, sharing, rollback and restore, over Patches, Companies and Auth. Destructive confirmation pages and deactivation/reactivation pick pages remain future work.
...
- **Portal → Patches, Companies, Auth, `core`**: reads the company inventory and manages patches through Patches, admits company viewers through Auth, and composes the shared app shell.
