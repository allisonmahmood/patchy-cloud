...
- Exception: an adapter relaying a wire refusal union, such as the CLI relaying `@patchy/api` refusal bodies, carries the decoded body in one tagged error and derives its message from the wire's `code`. Separate tags there would duplicate the wire union.
...
- `/api/*` is one `HttpApi` defined in `packages/api` (wire schemas, the `HttpApi`, the client), implemented by the server and consumed by the CLI. Serving owns patch addresses, internal content and `/healthz`; the server registers the root and catch-all together, using Portal for the signed-in root and Serving's signed-out door. Portal, Auth and Integrations mount their session pages through plain `HttpRouter` routes. Map domain errors onto declared `HttpApiError` responses at the handler; do not let an untyped failure decide a status code.
